...
from ui.theme import (
    ...
    COLOR_TEXT_PRIMARY, COLOR_ACCENT_HOVER, FONT_SIZE_LARGE
)
from core.lang import lang


class DownloadConfirmOverlay(QWidget):
    """Overlay that confirms overwriting an existing server jar."""

    def __init__(self, parent, blur_target, jar_name: str, confirm_callback, cancel_callback):
    ...
    def _build(self):
        ...
        title = QLabel(lang.get("ui.basic.download.overwrite.title"))
        title.setStyleSheet(
            f"font-size: {FONT_SIZE_LARGE}px; font-weight: bold;"
            f"color: {COLOR_ACCENT_HOVER};"
        )
        card_layout.addWidget(title)

        desc = QLabel(lang.get("ui.basic.download.overwrite.desc").format(self._jar_name))
        desc.setWordWrap(True)
        desc.setStyleSheet(f"color: {COLOR_TEXT_PRIMARY};")
        card_layout.addWidget(desc)

        hint = QLabel(lang.get("ui.basic.download.overwrite.hint"))
        ...
        overwrite_btn = QPushButton(lang.get("ui.basic.download.overwrite"))
        overwrite_btn.setStyleSheet(STYLE_BUTTON)
        overwrite_btn.clicked.connect(self._on_confirm)
